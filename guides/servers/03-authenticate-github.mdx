...
<Steps>
  <Step title="Copy the public key">
    If the terminal from A2 is still open, the key is already displayed. Otherwise, on the server:

    ```bash
    cat ~/.ssh/server_github_key.pub
    # Expected: one line starting with ssh-ed25519, ending with date stamp comment
    ```

    Copy the **entire line** — it starts with `ssh-ed25519` and ends with a comment like `server-github-20251030`.
